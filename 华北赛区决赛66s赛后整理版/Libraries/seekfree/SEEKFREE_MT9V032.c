...
uint8   Image_I[ROW][COL];      //图像数组
uint8   imagenew[ROW][COL];      //图像新数组
uint8   receive[3];
uint8   receive_num = 0;
...
void camera_init(void)
{
    ...
    dma_portx2buff_init(DMA_CH0, (void *)&C_IN_DATA(1), (void *)Image_I, C18, DMA_BYTE1, COL, DADDR_KEEPON);
...
{
	now_row = 0;
	dma_repeat(DMA_CH0,(void *)&C_IN_DATA(1),(void *)Image_I[0],COL);
}
...
void row_finished(void)
{
	...
	now_row++;
	if(now_row<ROW) dma_repeat(DMA_CH0,(void *)&C_IN_DATA(1),(void *)Image_I[now_row],COL);
...
void seekfree_sendimg_032(void)
{
   ...
   uart_putbuff(uart1, (uint8_t *)Image_I, ROW*COL);  //发送图像
	  //uart_putbuff(uart1, (uint8_t *)image, ROW_C*COL_C);  //发送图像
}
